...
<body>
   ...
   <table class="type08">
      ...
      <tbody>
         ...
         <tr>
            <td>${n.n}</td>
            <td><a href="/noticedetail/noticedetail?num=${n.num}">${n.title }</a></td>
            <td>${n.writer }</td>
            <td>${n.date }</td>
	...
	<c:if test="${authority eq '2'}">
      <div>
         ...
         <button class="btn btn-success btn-lg btn-block" onclick="location.href='/noticewrite/noticewrite.jsp'" style="float:right;" type="submit">글쓰기</button>
      </div>
   </c:if>
